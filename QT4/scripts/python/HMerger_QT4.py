...
import hou, os, glob, difflib, re

class HMerger (QtGui.QWidget):
    def __init__(self):
        ...
        self.ln_path = QtGui.QLineEdit()
        self.btn_B_file = QtGui.QToolButton()
        self.btn_B_file.setText('...')

        self.ch_conv = QtGui.QCheckBox('Convert to OBJ')
        self.ch_seq = QtGui.QCheckBox('Sequence')

        self.files_view = QtGui.QTreeWidget()
        ...
        self.files_view.setSelectionMode(QtGui.QAbstractItemView.MultiSelection)
        self.btn_get = QtGui.QPushButton()
        self.btn_selAll = QtGui.QPushButton()
        self.btn_selCls = QtGui.QPushButton()
        ptsrv=hou.getenv('SERVER')+'/Houdini_pipeline/merger_Nodes'
        ptsrv='d:/test'
    ...
    def setPath(self):
        ospath = QtGui.QFileDialog.getExistingDirectory(self, "Select Path", self.ln_path.text())
        if ospath:
    ...
    def updateList(self):      
        ...
        for i in range(0,len(fls),2):
            QtGui.QTreeWidgetItem(self.files_view,[fls[i],fls[i+1]])
